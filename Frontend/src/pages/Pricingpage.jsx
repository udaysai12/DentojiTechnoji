    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50">
        ...
        {isProcessingPayment && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4">
                <div className="bg-white rounded-2xl p-6 sm:p-8 text-center shadow-2xl max-w-sm w-full border border-gray-100">
                    <div className="relative mb-4 sm:mb-6">
                        <Loader2 className="w-10 h-10 sm:w-12 sm:h-12 text-[#4264D0] mx-auto animate-spin" />
        ...
        <div className="relative z-10 p-4 sm:p-6">
            ...
            <div className="max-w-6xl mx-auto mb-8 sm:mb-12">
                <div className="text-center mb-8 sm:mb-12">
                    <div className="inline-flex items-center gap-2 bg-[#4264D0]/10 text-[#4264D0] px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium mb-4 sm:mb-6 animate-fade-in">
                ...
                <div className={`border rounded-xl sm:rounded-2xl p-3 sm:p-4 mb-6 sm:mb-8 shadow-lg transition-all duration-300 ${
                    hasExistingHospital ? 'bg-blue-50/50 border-blue-200/50' : 'bg-green-50/50 border-green-200/50'
                }`}>
                    <div className="flex items-center gap-2 sm:gap-3">
                        <Shield className={`w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0 ${hasExistingHospital ? 'text-blue-600' : 'text-green-600'}`} />
                        <div className="flex-1 min-w-0">
                            <span className={`font-semibold text-sm sm:text-base ${hasExistingHospital ? 'text-blue-800' : 'text-green-800'}`}>
                                {hasExistingHospital ? 'Existing Admin - Renewal Plans' : 'New Admin - Welcome Plans'}
                            </span>
                            <p className={`${hasExistingHospital ? 'text-blue-700' : 'text-green-700'} text-xs sm:text-sm mt-1`}>
                                {hasExistingHospital 
            ...
            <div className="max-w-6xl mx-auto">
                {plans.length === 0 ? (
                    <div className="bg-red-50/50 border border-red-200/50 rounded-xl sm:rounded-2xl p-6 sm:p-8 text-center shadow-lg">
                        <AlertCircle className="w-10 h-10 sm:w-12 sm:h-12 text-red-500 mx-auto mb-3 sm:mb-4" />
                        <h3 className="text-lg sm:text-xl font-bold text-red-800 mb-2">No Pricing Plans Available</h3>
                        <p className="text-sm sm:text-base text-red-600 mb-4">Unable to load pricing information from the server.</p>
                        <button 
                            onClick={handleRetry}
                            className="bg-red-600 hover:bg-red-700 text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg font-medium transition-colors text-sm sm:text-base"
                        >
                            Try Again
                        </button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 mb-8 sm:mb-12">
                        {plans.map((plan, index) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className={`relative rounded-2xl sm:rounded-3xl border-2 transition-all duration-700 md:hover:scale-[1.02] md:hover:shadow-2xl group bg-gradient-to-br ${config.gradient} ${config.borderColor} ${
                                        isCurrentPlan ? 'ring-4 ring-green-300/60 shadow-2xl' : 'shadow-xl md:hover:shadow-2xl'
                                    } ${config.isPopular ? 'ring-2 ring-[#4264D0]/20' : ''} backdrop-blur-sm md:transform md:hover:-translate-y-2`}
                                    style={{
                                ...
                                >
                                    ...
                                    {config.isPopular && !isCurrentPlan && (
                                        <div className="absolute -top-4 sm:-top-6 left-1/2 transform -translate-x-1/2 z-10">
                                            <div className="bg-gradient-to-r from-[#4264D0] to-[#6B73FF] text-white px-4 sm:px-6 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-bold shadow-lg animate-bounce">
                                                {config.badge}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {isCurrentPlan && (
                                        <div className="absolute -top-3 sm:-top-4 right-3 sm:right-4 z-10">
                                            <div className="bg-green-500 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs font-bold shadow-lg animate-pulse">
                                    ...
                                    <div className="p-5 sm:p-8 h-full flex flex-col">
                                        ...
                                        <div className="mb-4 sm:mb-6">
                                            <div className="flex items-center justify-between mb-3 sm:mb-4">
                                                <span className={`text-xs sm:text-sm font-medium ${config.accentColor}`}>
                                            ...
                                            <p className="text-gray-600 text-base sm:text-lg">{config.tagline}</p>
                                        </div>
                                        ...
                                        <div className="mb-6 sm:mb-8">
                                            ...
                                            {plan.originalPrice && plan.planType !== 'Yearly Plan' && (
                                                <div className="mt-2">
                                                    <span className="text-gray-500 line-through text-xs sm:text-sm mr-2">{plan.originalPrice}</span>
                                                    {plan.savings && (
                                                        <span className="text-green-600 font-bold text-xs sm:text-sm bg-green-100 px-2 py-1 rounded-full">
                                                            {plan.savings}
                                                        </span>
                                                    )}
                                                </div>
                                        ...
                                        <div className="flex-1 mb-6 sm:mb-8">
                                            <div className="space-y-2.5 sm:space-y-3">
                                                {(plan.benefits || plan.features || []).slice(0, 6).map((feature, i) => (
                                                    <div key={i} className="flex items-start gap-2.5 sm:gap-3 text-gray-700">
                                        ...
                                        <div className="mt-auto">
                                            <button
                                                ...
                                                className={`w-full text-white px-5 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl cursor-pointer font-bold transition-all duration-300 md:transform md:hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none ${config.buttonStyle} shadow-lg md:hover:shadow-xl relative overflow-hidden group/btn text-sm sm:text-base`}
                                            >
                ...
                <div className="text-center mt-8 sm:mt-12 px-4">
                    <div className="inline-flex items-center gap-2 sm:gap-3 bg-green-50 border border-green-200 rounded-full px-4 sm:px-6 py-2.5 sm:py-3 max-w-2xl">
                        <div className="w-5 h-5 sm:w-6 sm:h-6 bg-green-500 rounded-full flex items-center justify-center flex-shrink-0">
                            <Check className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
                        </div>
                        <span className="text-green-800 text-xs sm:text-sm font-medium text-left">
