...
//                 disabled={isLoading || isDeleting}
//                 className="px-4 py-2 text-sm font-medium text-white bg-cyan-500 border border-transparent rounded-md hover:bg-cyan-600 disabled:opacity-50 disabled:cursor-not-allowed"
//               >
//                 {isLoading 
...
const EditStaffModal = ({ 
  ...
  useEffect(() => {
    if (isOpen && staffData) {
      // Extract name if it's a combined field
      let firstName = '';
      let lastName = '';
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      ...
      // Determine which API endpoint to use based on staffData source
      let url;
      let updateData = { ...formData };

      if (staffData.source === 'both') {
        // Update staff record (primary source)
        ...
        // Update receptionist record - need to map fields
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        {/* Footer with Save Changes and Cancel buttons */}
